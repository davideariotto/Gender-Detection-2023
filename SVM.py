...
def execute(DTR, LTR, DEV, LEV, allKFolds, evaluationLabels, single_fold, rangeK, rangeC, folder, pi_T = None):
    ...
    plot.plotDCF_SVM(rangeC, minDCF, "C", folder)
    print("\n\nPlot done \n\n")
...
def execute_find(DTR, LTR, DEV, LEV, allKFolds, evaluationLabels, single_fold, m, rangeK, d, rangec, rangeC):
    ...
    if(m==12):
        if(single_fold):
            ...
            plot.plotFindC(rangeC, minDCF, "C", "./SVM/SVM_poly/noPCA-kfold.png")
    else:
        if(single_fold):
            file_name = "./SVM/SVM_poly/PCA"+str(m)+"-singlefold.png"
            print(type(file_name))
            plot.plotFindC(rangeC, minDCF, "C", file_name)
        else:
            file_name = "./SVM/SVM_poly/PCA"+str(m)+"-kfold.png"
            plot.plotFindC(rangeC, minDCF, "C", file_name)
    print("\n\nPlot done \n\n")
...
def execute_find_RBF(DTR, LTR, DEV, LEV, allKFolds, evaluationLabels, single_fold, m, rangeK, rangeC, rangeGamma):
    ...
    if(m==12):
        ...
        if(single_fold):
            file_name = "./SVM/RBFKernel/PCA" + str(m) + "-singlefold.png"
            plot.plotDCF_RBF(rangeC, minDCF, "C", file_name)
        else:
            file_name = "./SVM/RBFKernel/PCA" + str(m) + "-kfold.png"
            plot.plotDCF_RBF(rangeC, minDCF, "C", file_name)
    print("\n\nPlot done \n\n")
...
def modifiedDualFormulationBalanced(DTR, LTR, C, K, piT):
    ...
    (x, f, d) = scipy.optimize.fmin_l_bfgs_b(dualObjectiveOfModifiedSVM, np.zeros(DTR.shape[1]), args=(Hij,), bounds=boxConstraint, factr=1.0)
    return np.sum((x*LTR).reshape(1, DTR.shape[1])*D, axis=1)
